...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		distance := distance(action)
		speed := meanSpeed(action, duration)
		calories := swimmingMeanSpeed(lengthPool, countPool, duration)
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	default:
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	energyRunning := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	//преобразуем км/ч в м/с
	speedMS := meanSpeed(action, duration) * kmhInMsec

	energyWalking := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMS, 2.0)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return energyWalking
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	energySwimming := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return energySwimming
}
